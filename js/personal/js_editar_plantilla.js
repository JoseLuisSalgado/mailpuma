$(document).ready(function(){
        ...
        $.ajax({   //AJAX para buscar plantilla en BD
            ...
            success: function(data){  //Respuesta servidor
                ...
                if(datos_tabla[0] == "error"){     //Si existe error en la BD
                    ...
                    texarea_herramientas(datos_tabla[3]);             //Genera textarea con recuadros de estilo para mensaje de email, (argumento es el mensaje de email, ya que asi lo requiere la libreria para mostrarlo en formulario)
    ...
    $("#namePlantilla, #asunto").keypress(function(e) {
        var code = (e.keyCode ? e.keyCode : e.which); 
        if(code == 13){ 
            validar_datos_actualizar_plantilla();
            return false; 
        } 
    });

    //Boton Guardar cambios, valida los datos del formulario
    $("#btn_update_plant").on("click", function(){
        validar_datos_actualizar_plantilla();
    });

    //Valida datos de formulario y actualiza la plantilla en BD
    function validar_datos_actualizar_plantilla(){
        ...
        if(nom_plantilla != ""){                                          //Validando campo nombre plantilla no sea vacio
            if(asunto != ""){                                          //Validando campo asunto del mensaje no sea vacio
                if(cadena_msje != ""){                                          //Validando campo mensaje no sea vacio
                    ...
                    $.ajax({
                        method:"POST",
                        url: "../class/actualizar_plantilla.php", 
                        data:{
                            id_plantilla: id_plant,
                            namePlantilla: nom_plantilla,
                            asunto: asunto,
                            box_mensaje: cadena_msje,
                        },  
                        success: function(data){  //Respuesta servidor
                            if(data == "plantilla_actualizada"){     //Si plantilla se agrego correctamente en BD

                                Swal.fire({
                                    icon: 'success',
                                    title: 'Se actualizó la plantilla "'+ nom_plantilla +'" de forma exitosa',
                                    confirmButtonText: 'Aceptar',
                    ...
                    $("#namePlantilla").focus();
                    Swal.fire({
                        title: 'El campo "Mensaje" está vacio',
                        text: "Favor de escirbir el mensaje del correo.",
                        confirmButtonText: 'Aceptar',
                        icon: 'warning',
                        confirmButtonColor: '#28a745'
                    })
                ...
                $("#namePlantilla").focus();
                Swal.fire({
                    title: 'El campo "Asunto" está vacio',
                    text: "Favor de registrar el asunto del mensaje.",
                    confirmButtonText: 'Aceptar',
                    icon: 'warning',
                    confirmButtonColor: '#28a745'
                })
            ...
            $("#namePlantilla").focus();
            Swal.fire({
                title: 'El campo "Nombre plantilla" está vacio',
                text: "Favor de registrar el nombre de la plantilla.",
                confirmButtonText: 'Aceptar',
                icon: 'warning',
                confirmButtonColor: '#28a745'
            })
    ...
    function texarea_herramientas(mensaje){
        $('#box_mensaje').summernote(
            //$('#box_mensaje').eq(0).html(mensaje),
            {
        ...
        $('#box_mensaje').html(mensaje));   //Se agrega el campo mensaje de BD al textarea del formulario para su modificacion
    }
    ...
    $("#btn_email_prueba").on("click", function(){
        ...
        Swal.fire({
            title: "¿A que cuenta desea recibir el correo de prueba?",
            input: "text",
    ...
    localStorage.removeItem("id_plant");           //Limpiando localstorage
